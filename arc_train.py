...
def arc_train(epoch, epoch_fn, opt, train_loader, discriminator, logger,
    ...
    logger.add_scalar('arc_train_loss', train_loss_epoch)
    logger.add_scalar('arc_train_acc', train_acc_epoch)

    assert np.isnan(train_loss_epoch) == False, 'ERROR. Found NAN in train_ARC.'

    # Reduce learning rate when a metric has stopped improving
    logger.add_scalar('train_lr', [param_group['lr'] for param_group in optimizer.param_groups][0])
