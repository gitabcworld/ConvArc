...
from models.siamese import Siamese,ContrastiveLoss
...
def train():
    ...
    ctrLoss = ContrastiveLoss()
    if opt.cuda:
        ctrLoss = ctrLoss.cuda()
    ...
    try:
        ...
        while iter_n < opt.n_iter:
            ...
            training_loss = ctrLoss(pred_train, Y_train.float())
            optimizer.zero_grad()
            ...
            if iter_n % opt.val_freq == 0:
                ...
                for i in range(opt.val_num_batches):
                    ...
                    loss_val = ctrLoss(pred_val, Y_val.float())
    ...
    for i in range(opt.test_num_batches):
        ...
        loss_test = ctrLoss(pred_test, Y_test.float())
