...
def peleeNet(pretrained=False, **kwargs):
    """
    Constructs a PeleeNet
    """
    model = PeleeNet(**kwargs)
    if pretrained:
        path = os.path.join(tempfile.gettempdir(), 'peleenet_acc7208.pth')
        if not os.path.isfile(path):
            urlModel = 'https://github.com/Robert-JunWang/PeleeNet/blob/master/weights/peleenet_acc7208.pth.tar?raw=true'
            print('Downloading peleeNet in %s to %s ...' % (urlModel,path))
        ...
        # change the keys in the dictionary and remove the "module." to load the pretrained weights
        state_dict_new = {}
        for key in state_dict.keys():
            state_dict_new[key[len('module.'):]] = state_dict[key]
